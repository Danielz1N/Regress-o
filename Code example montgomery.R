...
data <- data.frame(x.center,x,y)
data %>% ggplot(aes(x,y)) +
  geom_point(size=2,col="blue",alpha=0.6) + theme_bw()+
  labs(title="Hardwood concetration per Tensile strength",
       subtitle="Example 7.1 Montgomery (2012)", 
       y="Tensile strength", x="Hardwood concentration (%)")+
...
model  <- lm(y~poly(x.center,2,raw=T),data)

anova(model)

model2  <- lm(y~poly(x.center,1,raw=T),data)
...
envel.norm(model, conf=0.95, iden=6)
